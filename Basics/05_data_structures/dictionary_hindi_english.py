...
hindi_input = input("Enter a Hindi Word : ")
english_input = input("Enter the English word for that hindi Word : ")
hindi_english_dict.update({hindi_input: english_input})
...
combined_input = input("Enter a Hindi Word and also give its English translation : ") #Takes input() from user
hindi_word = combined_input.split(" ").pop(0) #if the user gives "ram" and "shyam" then this split make it list of strings then the pop() delete the first element and return it and we just capture that return value and store it in a variable
english_word = combined_input.split(" ").pop(1) #same for second variable
