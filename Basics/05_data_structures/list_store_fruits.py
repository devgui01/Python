#1. Write a program to store a fruit in a list entered by the user.
user_input = input("Enter a Fruit Name : ") #User input the 7 fruits
fruits_list = [] #Creates a Blank list
fruits_list.append(user_input) #Append the userinput fruits in the empty list
print(fruits_list) #Print the list



# 2. Write a program to store seven fruits in a list entered by the user.
fruits_list = []
fruit1 = input("Enter a Fruit Name : ")
fruit2 = input("Enter a Fruit Name : ")
fruit3 = input("Enter a Fruit Name : ")
fruit4 = input("Enter a Fruit Name : ")
fruit5 = input("Enter a Fruit Name : ")
fruit6 = input("Enter a Fruit Name : ")
fruit7 = input("Enter a Fruit Name : ")
fruits_list.append(fruit1)
fruits_list.append(fruit2)
fruits_list.append(fruit3)
fruits_list.append(fruit4)
fruits_list.append(fruit5)
fruits_list.append(fruit6)
fruits_list.append(fruit7)
print(fruits_list)
